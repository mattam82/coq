...
let is_sort_variable sigma s =
  ...
  | SProp | Prop | Set -> None
  | Type u | Erased u | QSort (_, u) -> match Univ.Universe.level u with
    | None -> None
    | Some l ->
...
(* ps = parameter list *)
let typecheck_params_and_fields def ~poly ~cumulative udecl ps (records : DataI.t list) : tc_result =
...
let pre_process_structure udecl kind ~poly ~cumulative (records : Ast.t list) =
  ...
  let impargs, univs, params, data =
    ...
    *)
    Vernacstate.System.protect (fun () ->
        typecheck_params_and_fields (kind = Class true) ~poly ~cumulative udecl ps data) ()
  in
...
let definition_structure ~flags udecl kind ~primitive_proj (records : Ast.t list)
  ...
  let impargs, params, univs, projections_kind, data, indlocs =
    pre_process_structure udecl kind ~poly ~cumulative records
  in
