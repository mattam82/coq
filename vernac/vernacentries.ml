...
module DefAttributes = struct
  ...
  *)
  ...
  let def_attributes_gen ?(coercion=false) ?(discharge=NoDischarge,"","") () =
    ...
    (locality ++ user_warns_with_use_globref_instead ++ poly SortPolyFlags.Definition ++ program ++
...
let preprocess_defclass ~atts udecl (id, bl, c, l) =
  let poly, mode =
    Attributes.(parse Notations.(poly SortPolyFlags.Definition ++ mode_attr) atts)
  in
  let flags = {
