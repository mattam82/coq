...
module DefAttributes = struct
  ...
  *)
  ...
  let def_attributes_gen ?(coercion=false) ?(discharge=NoDischarge,"","") ?(ass_or_def=SortPolyFlags.Definition) () =
...
| SubClass ->
  Some (ComCoercion.add_subclass_hook ~poly:(SortPolyFlags.level_polymorphic poly) ~reversible)
...
let preprocess_defclass ~atts udecl (id, bl, c, l) =
  ...
  in
  let poly = SortPolyFlags.make ~level_polymorphic:poly ~cumulative ~sort_polymorphic:sort_poly in
  let flags = {
...
let vernac_instance_program ~atts ~pm name bl t props info =
  ...
  let locality, poly =
    Attributes.(parse (Notations.(hint_locality ++ poly SortPolyFlags.Definition))) atts
  in
...
let vernac_declare_instance ~atts id bl inst pri =
  ...
  let ((program, locality), poly) =
    Attributes.(parse (Notations.(program ++ hint_locality ++ poly SortPolyFlags.Definition))) atts
  in
  Classes.declare_new_instance ~program_mode:program ~locality ~poly id bl inst pri

let vernac_context ~atts ctx =
  let (program_mode, poly) = Attributes.(parse (Notations.(program ++ poly SortPolyFlags.Assumption))) atts in
...
let translate_pure_vernac ?loc ~atts v = let open Vernactypes in match v with
  ...
  | VernacDefinition ((discharge,kind as dkind),lid,DefineBody (bl,red_option,c,typ)) ->
    ...
    if refine then
      vtopenproof(fun () ->
        ...
        ~ass_or_def:SortPolyFlags.Definition ~atts
      ...
      vtmodifyprogram (fun ~pm ->
        ...
        ~ass_or_def:SortPolyFlags.Definition vernac_definition ~pm dkind lid bl red_option c typ)
  ...
  | VernacAssumption ((discharge,kind),nl,l) ->
    vtdefault(fun () ->
        with_def_attributes ~ass_or_def:SortPolyFlags.Assumption ~atts
          ~discharge:(discharge,
  ...
  | VernacSymbol l ->
    vtdefault (fun () ->
      let (unfold_fix, poly) =
        Attributes.(parse Notations.(unfold_fix ++ poly SortPolyFlags.Assumption)) atts
      in
  ...
  | VernacAddRewRule (id, c) ->
    vtdefault (fun () ->
        let poly = Attributes.(parse (poly SortPolyFlags.Assumption)) atts in
