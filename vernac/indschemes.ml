...
let define ~poly ~cumulative ?loc name sigma c types =
  let poly =
    SortPolyFlags.make ~level_polymorphic:poly
      ~sort_polymorphic:false (* FIXME sort_poly not supported *)
      ~cumulative
  in
  let univs = Evd.univ_entry ~poly ?variances:(if SortPolyFlags.cumulative poly then Some Infer_variances else None) sigma in
