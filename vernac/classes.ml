...
let declare_instance_constant iinfo global impargs ?hook (name:lident) udecl poly_flags sigma term termtype =
  let kind = Decls.(IsDefinition Instance) in
  let cinfo = Declare.CInfo.make ?loc:name.loc ~name:name.v ~impargs ~typ:(Some termtype) () in
  let info = Declare.Info.make ~kind ~poly_flags ~udecl () in
...
let do_declare_instance sigma ~locality poly_flags k ctx ctx' pri udecl impargs subst (name:lident) =
  ...
  let sigma, entry = Declare.prepare_parameter ~poly_flags sigma ~udecl ~types:termtype in
...
let declare_instance_program pm env sigma ~locality poly_flags {CAst.v=name;loc} pri impargs udecl term termtype =
  ...
  let info = Declare.Info.make ~udecl ~poly_flags ~kind ~hook () in
  let pm, _ =
    Declare.Obls.add_definition ~pm ~info ~cinfo ~opaque:false ~uctx ~body obls
  in pm

let declare_instance_open sigma ?hook ~tac ~locality poly_flags (id:lident) pri impargs udecl ids term termtype =
  ...
  let info = Declare.Info.make ~hook ~kind ~udecl ~poly_flags () in
...
let do_instance_resolve_TC poly_flags term termtype sigma env =
  ...
  let sigma = Evd.minimize_universes ~to_type:(not (SortPolyFlags.sort_polymorphic poly_flags)) ~partial:(Option.is_empty term) sigma in
...
let do_instance_interactive env env' sigma ?hook ~tac ~locality poly_flags cty k ctx ctx' pri decl imps subst id opt_props =
  let term, termtype, sigma = match opt_props with
    ...
    | None ->
      ...
      in
      let termtype, sigma = do_instance_resolve_TC poly_flags term termtype sigma env in
      term, termtype, sigma
  in
  Flags.silently (fun () ->
      declare_instance_open sigma ?hook ~tac ~locality poly_flags
        id pri imps decl (List.map RelDecl.get_name ctx) term termtype)
    ()

let do_instance env env' sigma ?hook ~locality poly_flags cty k ctx ctx' pri decl imps subst id props =
  let term, termtype, sigma =
    interp_props ~program_mode:false env' cty k ctx ctx' subst sigma props
  in
  let termtype, sigma = do_instance_resolve_TC poly_flags (Some term) termtype sigma env in
  Pretyping.check_evars_are_solved ~program_mode:false env sigma;
  declare_instance_constant pri locality imps ?hook id decl poly_flags sigma term termtype

let do_instance_program ~pm env env' sigma ?hook ~locality poly_flags cty k ctx ctx' pri decl imps subst id opt_props =
  let term, termtype, sigma =
  ...
  if not (Evd.has_undefined sigma) && not (Option.is_empty opt_props) then
    let () = declare_instance_constant pri locality imps ?hook id decl poly_flags
               sigma term termtype in
    pm
  else
    declare_instance_program pm env sigma ~locality poly_flags id pri imps decl term termtype
...
let interp_instance_context ~program_mode ~poly_flags env ctx pl tclass =
  let sigma, decl = interp_sort_poly_decl_opt env pl in
  let sigma, (impls, ((env', ctx), imps, _locs)) = interp_context_evars ~program_mode ~poly_flags env sigma ctx in
  let flags = Pretyping.{ all_no_fail_flags with program_mode ; poly_flags } in
...
let new_instance_common ~program_mode ~poly_flags env instid ctx cl =
  let (instid, pl) = instid in
  let sigma, k, u, cty, ctx', ctx, imps, subst, decl =
    interp_instance_context ~program_mode ~poly_flags env ctx pl cl
  in
...
let new_instance_interactive ~locality ~poly_flags instid ctx cl
    ?(tac:unit Proofview.tactic option) ?hook
    pri opt_props =
  let env = Global.env() in
  let id, env', sigma, k, u, cty, ctx', ctx, imps, subst, decl =
    new_instance_common ~program_mode:false ~poly_flags env instid ctx cl in
  id, do_instance_interactive env env' sigma ?hook ~tac ~locality poly_flags
    cty k ctx ctx' pri decl imps subst id opt_props

let new_instance_program ~locality ~pm ~poly_flags instid ctx cl opt_props ?hook pri =
  let env = Global.env() in
  let id, env', sigma, k, u, cty, ctx', ctx, imps, subst, decl =
    new_instance_common ~program_mode:true ~poly_flags env instid ctx cl in
  let pm =
    do_instance_program ~pm env env' sigma ?hook ~locality poly_flags
      cty k ctx ctx' pri decl imps subst id opt_props in
  pm, id

let new_instance ~locality ~poly_flags instid ctx cl props ?hook pri =
  let env = Global.env() in
  let id, env', sigma, k, u, cty, ctx', ctx, imps, subst, decl =
    new_instance_common ~program_mode:false ~poly_flags env instid ctx cl in
  do_instance env env' sigma ?hook ~locality poly_flags
    cty k ctx ctx' pri decl imps subst id props;
  id

let declare_new_instance ~locality ~program_mode ~poly_flags instid ctx cl pri =
  let env = Global.env() in
  let (instid, pl) = instid in
  let sigma, k, u, cty, ctx', ctx, imps, subst, decl =
    interp_instance_context ~program_mode ~poly_flags env ctx pl cl
  in
  do_declare_instance sigma ~locality poly_flags k ctx ctx' pri decl imps subst instid
