...
let do_instance_resolve_TC ~poly ~sort_poly term termtype sigma env =
...
let do_instance_interactive env env' sigma ?hook ~tac ~locality ~poly ~sort_poly ~cumulative cty k ctx ctx' pri decl imps subst id opt_props =
  let term, termtype, sigma = match opt_props with
    ...
    | None ->
      ...
      in
      let termtype, sigma = do_instance_resolve_TC ~poly ~sort_poly term termtype sigma env in
      term, termtype, sigma
  in
  Flags.silently (fun () ->
...
let do_instance env env' sigma ?hook ~locality ~poly ~sort_poly ~cumulative cty k ctx ctx' pri decl imps subst id props =
  let term, termtype, sigma =
    interp_props ~program_mode:false env' cty k ctx ctx' subst sigma props
  in
  let termtype, sigma = do_instance_resolve_TC ~poly ~sort_poly (Some term) termtype sigma env in
...
let interp_instance_context ~program_mode ~poly ~sort_poly env ctx pl tclass =
  let sigma, decl = interp_sort_poly_decl_opt env pl in
  let sigma, (impls, ((env', ctx), imps, _locs)) = interp_context_evars ~program_mode ~poly ~sort_poly env sigma ctx in
...
let new_instance_common ~program_mode ~poly ~sort_poly env instid ctx cl =
