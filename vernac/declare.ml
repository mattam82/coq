...
let add_mono_univ_uctx_for_derived ctx' e =
  let open UState in
  match e.universes_entry_universes with
  | UState.Monomorphic_entry ctx ->
    { e with universes_entry_universes = UState.Monomorphic_entry (Univ.ContextSet.union ctx ctx') }
  | UState.Polymorphic_entry (ctx, _variances) ->
...
let extend_variances univs =
  ...
  | Polymorphic_entry (uctx, variances) ->
    ...
    let extend vars =
      ...
      else UVars.Variances.of_array (Array.append avars (Array.make (ulen - Array.length avars) (UVars.(VariancePos.make Variance.Invariant Position.InTerm))))
    in
...
let finish_derived pinfo uctx entries =
  ...
  let _, _, refs, _ =
    List.fold_left2 (fun (i, subst, refs, used_univs) CInfo.{name; impargs} entry ->
      ...
      let uctx' = UState.restrict uctx used_univs in
      let ivariances = UnivMinim.empty_level_variances in
      let entry = { entry with proof_entry_universes = UState.check_univ_decl ~poly uctx' ivariances udecl } in
