  ...
  let hook, impls =
    if len > 1 then
      let hook { Declare.Hook.S.dref; uctx; obls; _ } =
        ...
        let univs = UState.check_sort_poly_decl ~poly ~kind:SortPolyFlags.Definition uctx udecl in
        let h_body =
...
let interp_fixpoint_short rec_order fixpoint_exprl =
  ...
  let (_, _, sigma),(fix, _, _) = interp_mutual_definition ~program_mode:false ~poly:SortPolyFlags.default ~function_mode:true env (CFixRecOrder rec_order) fixpoint_exprl in
  (* Instantiate evars and check all are resolved *)
  let sigma = Evarconv.solve_unif_constraints_with_heuristics env sigma in
...
let do_mutually_recursive ?pm ~refine ~program_mode ?(use_inference_hook=false) ?scope ?clearbody ~kind ~poly ?typing_flags ?user_warns ?using (rec_order, fixl)
  ...
  | Some pm ->
    ...
    let sigma = if SortPolyFlags.level_polymorphic poly then sigma else Evd.fix_undefined_variables sigma in
