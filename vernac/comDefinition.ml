...
let do_definition ?loc ?hook ~name ?scope ?clearbody ~poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  let info = Declare.Info.make ?loc ?scope ?clearbody ~kind ?hook ~udecl ~poly ~cumulative ?typing_flags ?user_warns () in
...
let do_definition_program ?loc ?hook ~pm ~name ~scope ?clearbody ~poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  let pm, _ =
    ...
    let info = Declare.Info.make ?loc ~udecl ~scope ?clearbody ~poly ~cumulative ~kind ?hook ?typing_flags ?user_warns () in
