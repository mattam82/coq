...
let interp_definition ~program_mode ~poly_flags env evd impl_env bl red_option c ctypopt =
  ...
  let evd, (impls, ((env_bl, ctx), imps1, _locs)) = interp_context_evars ~program_mode ~poly_flags ~impl_env env evd bl in
  (* Build the type *)
  ...
  let evd, c, imps, tyopt =
    match tyopt with
    | None ->
      let evd, (c, impsbody) = interp_constr_evars_impls ~program_mode ~poly_flags ~impls env_bl evd c in
      evd, c, imps1@impsbody, None
    | Some (ty, impsty) ->
      let evd, (c, impsbody) = interp_casted_constr_evars_impls ~program_mode ~poly_flags ~impls env_bl evd c ty in
...
let interp_statement ~program_mode env evd ~(flags : Pretyping.inference_flags) ~scope name bl typ  =
  let poly_flags = Pretyping.(flags.poly_flags) in
  let evd, (impls, ((env, ctx), imps, _locs)) = Constrintern.interp_context_evars ~poly_flags ~program_mode env evd bl in
...
let do_definition ?loc ?hook ~name ?scope ?clearbody ~poly_flags ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  let evd, (body, types), impargs =
    interp_definition ~program_mode ~poly_flags env evd empty_internalization_env bl red_option c ctypopt
  in
  let kind = Decls.IsDefinition kind in
  let cinfo = Declare.CInfo.make ?loc ~name ~impargs ~typ:types () in
  let info = Declare.Info.make ?scope ?clearbody ~kind ?hook ~udecl ~poly_flags ?typing_flags ?user_warns () in
  let _ : Names.GlobRef.t =
    Declare.declare_definition ~info ~cinfo ~opaque:false ~body ?using evd
  in ()

let do_definition_program ?loc ?hook ~pm ~name ~scope ?clearbody ~poly_flags ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  let env = Global.env() in
  let env = Environ.update_typing_flags ?typing_flags env in
  (* Explicitly bound universes and constraints *)
  let evd, udecl = interp_sort_poly_decl_opt env udecl in
  let evd, (body, types), impargs =
    interp_definition ~program_mode:true ~poly_flags env evd empty_internalization_env bl red_option c ctypopt
  in
  let body, typ, uctx, _, obls = Declare.Obls.prepare_obligations ~name poly_flags ~body ?types env evd in
  Evd.check_sort_poly_decl_early poly_flags ~with_obls:true (Evd.from_ctx uctx) udecl [body; typ];
  let pm, _ =
    let cinfo = Declare.CInfo.make ?loc ~name ~typ ~impargs () in
    let info = Declare.Info.make ~udecl ~scope ?clearbody ~poly_flags ~kind ?hook ?typing_flags ?user_warns () in
    Declare.Obls.add_definition ~pm ~info ~cinfo ~opaque:false ~body ~uctx ?using obls
  in pm

let do_definition_interactive ?loc ~program_mode ?hook ~name ~scope ?clearbody ~poly_flags ~typing_flags ~kind ?using ?user_warns udecl bl t =
  let env = Global.env () in
  let env = Environ.update_typing_flags ?typing_flags env in
  let flags = Pretyping.{ all_no_fail_flags with program_mode; poly_flags } in
  ...
  let evd = Evd.minimize_universes ~to_type:(not (SortPolyFlags.sort_polymorphic poly_flags)) ~partial:udecl.sort_poly_decl_extensible_instance evd in
  Pretyping.check_evars_are_solved ~program_mode env evd;
  let typ = EConstr.to_constr evd typ in
  Evd.check_sort_poly_decl_early poly_flags ~with_obls:false evd udecl [typ];
  let typ = EConstr.of_constr typ in
  let info = Declare.Info.make ?hook ~poly_flags ~scope ?clearbody ~kind ~udecl ?typing_flags ?user_warns () in
  let cinfo = Declare.CInfo.make ?loc ~name ~typ ~args ~impargs () in
  let evd = if SortPolyFlags.level_polymorphic poly_flags then evd else Evd.fix_undefined_variables evd in
  Declare.Proof.start_definition ~info ~cinfo ?using evd

let do_definition_refine ?loc ?hook ~name ~scope ?clearbody ~poly_flags ~typing_flags ~kind ?using ?user_warns udecl bl c ctypopt =
  let env = Global.env() in
  let env = Environ.update_typing_flags ?typing_flags env in
  (* Explicitly bound universes and constraints *)
  let evd, udecl = interp_sort_poly_decl_opt env udecl in
  let evd, (body, typ), impargs =
    interp_definition ~program_mode:false ~poly_flags env evd empty_internalization_env bl None c ctypopt
  in
