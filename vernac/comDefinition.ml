...
let interp_definition ~program_mode ~poly ~sort_poly env evd impl_env bl red_option c ctypopt =
...
let do_definition ?loc ?hook ~name ?scope ?clearbody ~poly ~sort_poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  let evd, (body, types), impargs =
    interp_definition ~program_mode ~poly ~sort_poly env evd empty_internalization_env bl red_option c ctypopt
  in
...
let do_definition_program ?loc ?hook ~pm ~name ~scope ?clearbody ~poly ~sort_poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  Evd.check_sort_poly_decl_early ~poly ~sort_poly ~cumulative ~with_obls:true (Evd.from_ctx uctx) udecl [body; typ];
  let pm, _ =
...
let do_definition_interactive ?loc ~program_mode ?hook ~name ~scope ?clearbody ~poly ~sort_poly ~cumulative ~typing_flags ~kind ?using ?user_warns udecl bl t =
  ...
  let evd = Evd.minimize_universes ~to_type:(not sort_poly) ~partial:udecl.sort_poly_decl_extensible_instance evd in
  Pretyping.check_evars_are_solved ~program_mode env evd;
  let typ = EConstr.to_constr evd typ in
  Evd.check_sort_poly_decl_early ~poly ~sort_poly ~cumulative ~with_obls:false evd udecl [typ];
...
let do_definition_refine ?loc ?hook ~name ~scope ?clearbody ~poly ~sort_poly ~typing_flags ~kind ?using ?user_warns udecl bl c ctypopt =
  ...
  let evd, (body, typ), impargs =
    interp_definition ~program_mode:false ~poly ~sort_poly env evd empty_internalization_env bl None c ctypopt
  in
