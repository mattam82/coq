...
let do_definition_program ?loc ?hook ~pm ~name ~scope ?clearbody ~poly ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  Evd.check_sort_poly_decl_early ~poly ~with_obls:true (Evd.from_ctx uctx) udecl [body; typ];
  let pm, _ =
...
let do_definition_interactive ?loc ~program_mode ?hook ~name ~scope ?clearbody ~poly ~typing_flags ~kind ?using ?user_warns udecl bl t =
  ...
  let evd = Evd.minimize_universes ~to_type:(not (SortPolyFlags.sort_polymorphic poly)) ~partial:udecl.sort_poly_decl_extensible_instance evd in
  ...
  let evd = if SortPolyFlags.level_polymorphic poly then evd else Evd.fix_undefined_variables evd in
