...
let local_binders_of_decls ~poly l =
  let coercions, l =
    List.fold_left_map (fun coercions (is_coe,(idl,c)) ->
      ...
      let decl = if SortPolyFlags.level_polymorphic poly then
...
let interp_context_gen scope ~program_mode ~poly ~kind ~autoimp_enable ~coercions env sigma l =
  ...
  let sigma, ctx = Evarutil.finalize ~to_type:(not (SortPolyFlags.sort_polymorphic poly)) ~partial:true sigma @@ fun nf ->
...
let interp_context env sigma ctx =
  ...
  let poly = SortPolyFlags.default in
