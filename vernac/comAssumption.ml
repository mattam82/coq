...
let declare_global ~coe ~try_assum_as_instance ~local ~kind ?user_warns ~univs ?variances ~impargs ~inline
...
let declare_context ~try_global_assum_as_instance ~scope ~univs ?variances ?user_warns ~inline ctx =
  let fn i subst d =
    ...
    let refu = match scope with
      | Locality.Discharge -> declare_local ~coe ~try_assum_as_instance:true ~kind ~univs ?variances ~impargs ~impl ~name:name.CAst.v b t
