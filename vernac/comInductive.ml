...
let include_constructor_argument evd ~poly ~ctor_sort ~inductive_sort =
  if SortPolyFlags.level_polymorphic  poly then
...
let inductive_levels env evd ~poly ~indnames ~arities_explicit arities ctors =
  ...
  let candidates = if SortPolyFlags.sort_polymorphic poly then [] else prop_lowering_candidates evd ~arities_explicit inds in
...
let nontemplate_univ_entry ~poly sigma udecl =
  let sigma = Evd.collapse_sort_variables ~to_type:(not (SortPolyFlags.sort_polymorphic poly)) sigma in
  let UState.{ universes_entry_universes = univ_entry; universes_entry_binders = ubinders } =
    Evd.check_sort_poly_decl ~poly sigma ~kind:SortPolyFlags.Definition udecl in
...
match user_template, SortPolyFlags.level_polymorphic poly with
| Some true, true ->
...
let interp_mutual_inductive_constr ~sigma ~flags ~udecl ~ctx_params ~indnames ~arities_explicit ~arities ~template_syntax ~constructors ~env_ar_params ~private_ind =
  ...
  debug Pp.(fun () -> str"interp_mutual, cumulative = " ++ bool (SortPolyFlags.cumulative poly));
  ...
  let sigma, (default_dep_elim, arities) =
    inductive_levels env_ar_params sigma ~poly
  ...
  let sigma = UnivVariances.register_universe_variances_of_inductive ~cumulative:(SortPolyFlags.cumulative poly) env_ar_params sigma ~udecl ~params:ctx_params ~arities ~constructors in
  let sort_poly = SortPolyFlags.sort_polymorphic poly in
...
let interp_mutual_inductive_gen env0 ~flags udecl (uparamsl,paramsl,indl) notations ~private_ind =
  ...
  let poly = SortPolyFlags.level_polymorphic flags.poly in
