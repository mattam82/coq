...
let interp_definition pl bl p red_option c ctypopt =
  ...
  let imps,pl,ce =
	...
	let pl, uctx = Univdecls.check_univ_decl evd decl in
	...
	let pl, uctx = Univdecls.check_univ_decl ctx decl in
  ...
  in
	...
	Obligations.eterm_obligations env ident evd 0 c typ
      ...
      ignore(Obligations.add_definition
          ident ~term:c cty ctx ?pl:pl ~implicits:imps ~kind:k ~hook obls)
...
let declare_fixpoint local poly ((fixnames,fixdefs,fixtypes),pl,ctx,fiximps) indexes ntns =
  if List.exists Option.is_empty fixdefs then
    ...
    let pl, ctx = Univdecls.check_univ_decl evd pl in
...
let do_program_recursive local p fixkind fixl ntns =
  ...
  in
  let pl = if pl.univdecl_extensible_instance then None else Some pl.univdecl_instance in
