...
Section elim.
  ...
  :=
    ...
    Fail Definition ty_elim : forall Γ (t : Ty Γ), TyM Γ (con_elim Γ) t :=
      fix F (c : Con) : ConM c :=
        match c return (ConM c) with
        | empty => f
        | ext g t => f1 g (F g) t (F0 g t)
...
| tsubst Γ Γ' (s : ConMap Γ Γ') A (t : @Term Γ' A) : @Term Γ (tysubst _ _ s A)
| v Γ A : Term (p Γ A)
| app Γ A B (t : Term (pi Γ A B)) (u : Term A) : Term (tysubst _ _ (extM _ _ u) B)
| utt Γ : Term (Uunit Γ)
