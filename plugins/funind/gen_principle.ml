...
let register_struct is_rec (rec_order, fixpoint_exprl) =
  ...
  | [{Vernacexpr.fname; univs; binders; rtype; body_def}] when not is_rec ->
    ...
    ComDefinition.do_definition ?loc:fname.CAst.loc ~name:fname.CAst.v
      ~sort_poly:false ~poly:false ~cumulative:false
  ...
  | _ ->
     let pm, p = ComFixpoint.do_mutually_recursive ~refine:false ~program_mode:false
                   ~poly:false ~sort_poly:false ~cumulative:false ~kind:(IsDefinition Fixpoint) (CFixRecOrder rec_order, fixpoint_exprl) in
