...
let build_functional_principle env (sigma : Evd.evar_map) old_princ_type sorts funs
  ...
  let body, typ, univs, _safe, _uctx =
    Declare.build_by_tactic env ~uctx ~poly:SortPolyFlags.default ~typ ftac
  in
    ...
    let hook new_principle_type _ =
      if Option.is_empty sorts then (
        ...
        let register_with_sort sort =
          ...
          let univs = Evd.univ_entry ~poly:SortPolyFlags.default evd' in
...
let register_struct is_rec (rec_order, fixpoint_exprl) =
  ...
  | _ ->
     let pm, p = ComFixpoint.do_mutually_recursive ~refine:false ~program_mode:false
                   ~poly:SortPolyFlags.default ~kind:(IsDefinition Fixpoint) (CFixRecOrder rec_order, fixpoint_exprl) in
