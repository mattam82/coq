...
let () =
  let interp ?loc poly_flags env sigma tycon (ids, tac) =
    ...
    in
    let c, sigma = Proof.refine_by_tactic ~name poly_flags (GlobEnv.renamed_env env) sigma concl tac in
...
let () =
  let interp ?loc poly_flags env sigma tycon (kind,id) =
