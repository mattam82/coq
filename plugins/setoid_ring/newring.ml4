...
let exec_tactic env evd n f args =
  ...
  let gls =
	...
	tclTHEN (Refiner.tclEVARS (Evd.merge sigma evd))
	  (Tacinterp.eval_tactic(ltac_call f (args@[getter]))) gl)
      (dummy_goal env) in
...
let add_theory name (sigma,rth) eqth morphth cst_tac (pre,post) power sign div =
  ...
  let lemma1 = decl_constant (Id.to_string name^"_ring_lemma1") ctx lemma1 in
  let lemma2 = decl_constant (Id.to_string name^"_ring_lemma2") ctx lemma2 in
  let cst_tac =
...
let ring_lookup (f:glob_tactic_expr) lH rl t gl =
  ...
  tclTHEN (Refiner.tclEVARS !evdref) (ltac_apply f (ring@[lH;rl])) gl
...
let add_field_theory name (sigma,fth) eqth morphth cst_tac inj (pre,post) power sign odiv =
  ...
  let cond_lemma =
    match inj with
      ...
      | None -> params.(7) in
  let lemma1 = decl_constant (Id.to_string name^"_field_lemma1") ctx lemma1 in
  let lemma2 = decl_constant (Id.to_string name^"_field_lemma2") ctx lemma2 in
  let lemma3 = decl_constant (Id.to_string name^"_field_lemma3") ctx lemma3 in
  let lemma4 = decl_constant (Id.to_string name^"_field_lemma4") ctx lemma4 in
  let cond_lemma = decl_constant (Id.to_string name^"_lemma5") ctx cond_lemma in
  let cst_tac =
    interp_cst_tac env sigma morphth kind (zero,one,add,mul,opp) cst_tac in
  let pretac =
    match pre with
