...
let make_eq_congr sigma =
  new_global sigma (Coqlib.build_coq_eq_data ()).Coqlib.congr
...
let get_rew_prf evars r = match r.rew_prf with
  ...
  | RewEq (p, pty, t1, t2, c, rel, car) ->
     if is_eq_abs p then evars, (mkApp (rel, [| car |]), c)
       ...
       let prf = mkApp (eq_congr,
			[| car; pty; pred; t1; t2; c |])
       in evars, ((mkApp (rel, [| pty |])), prf)
    ...
    let cstr' = Option.map (fun c -> (ty, Some c)) cstr in
	  ...
	  in
		...
		| Success r ->
			...
			rew_prf = prf; rew_evars = r.rew_evars; rew_decls = [] }
		    in
		    let res =
		      match prf with
		      | RewPrf (rel, prf) ->
...
| StratConstr (c, true, ai, ipat) -> prc c
| StratConstr (c, false, ai, ipat) -> str "<-" ++ spc () ++ prc c
