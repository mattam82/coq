...
sig
  ...
  (** Type of merging function when merging two stores having values for
    the same field (used in [merge] only). At least one of the
    arguments is [Some x] for some [x]. *)
  ...
  (** [merge s1 s2] merges the fields of [s1] into [s2].
      ...
      of the field, called with optional values of [s1] and [s2], at least
      one of those being [Some _]. *)
