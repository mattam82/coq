...
let make_hint_clenv poly ?len (c, cty, ctx) gl =
  ...
  in
  check_typeable_in_env env c; (* raise NotFound if variables are missing *)
  ...
  let tactic = function
    ...
    | Unfold_nth c ->
      Proofview.V82.tactic (fun gl ->
       ...
       tclPROGRESS (Proofview.V82.of_tactic
		    (reduce (Unfold [AllOccurrences false,c])
                            Locusops.onConcl)) gl
