...
let extend_evars subst c =
  let open Term in
  let rec aux c =
    match kind_of_term c with
    | Evar (ev, args) ->
       (try let args' = Evar.Map.find ev subst in
            mkEvar (ev, args')
        with Not_found -> c)
    | _ -> Constr.map aux c
  in aux c

let move_to_goal_hyps vars ctx hyps subst evi =
  let hypsids =
    Array.fold_left (fun acc h -> Id.Set.union acc (collect_vars h))
                    Id.Set.empty hyps
  in
  (* The hint depends on less variables than available in the goal,
     extend it to the whole goal hypotheses *)
  if Id.Set.subset hypsids vars then
    { evi with Evd.evar_hyps = ctx;
               Evd.evar_concl = extend_evars subst evi.Evd.evar_concl  } (* Evar filter? *)
  else
    (* The hint subgoal cannot be typechecked in this smaller environment *)
    raise Not_found
	
let subst_evars subst c =
  let open Term in
  let rec aux c =
    match kind_of_term c with
    | Evar (evk, args) ->
       (try mkEvar (Evar.Map.find evk subst, Array.map (Constr.map aux) args)
...
let isEvarKey k c =
  ...
  | Term.Evar (k', _) -> Evar.equal k k'
  | _ -> false

let merge_hint_sigma sigma sigma' clenv =
  let holes = clenv_holes clenv in
  let subst, sigma =
    Evd.fold_undefined (fun ev evi (subst, evd') ->
      if List.exists (fun h -> isEvarKey ev h.hole_evar) holes then
        let evi = Evarutil.nf_evar_info sigma' evi in
        let evi' = Evd.map_evar_info (subst_evars subst) evi in
        let evd',ev' = Evd.new_evar evd' evi' in
        let subst = Evar.Map.add ev ev' subst in
        (subst, evd')
      else (subst, evd')) sigma' (Evar.Map.empty, sigma)
  in
  let clenv' = clenv_map (subst_evars subst) clenv in
  subst, clenv', sigma

let connect_hint_clenv poly (c, _, ctx) (sigma', clenv) gl =
  ...
  let hypsc = named_context_of_val hyps in
  let inst = Array.map_of_list (fun h -> Constr.mkVar (Context.Named.Declaration.get_id h)) hypsc in
  let vars = Context.Named.to_vars hypsc in
  let subst, clenv, evd = merge_hint_sigma sigma sigma' clenv in
  let c = subst_evars subst c in
  let subst = ref Evar.Map.empty in
  let add_hole f (evd, holes) h =
    let ev, hyps' = Term.destEvar h.hole_evar in
    let evi = Evd.find evd ev in
    (** The environment of the subgoal is the same as the hint's subgoal 
        (the hint is local, no intro/clear was done) *)
    let evi' =
      if Environ.eq_named_context_val hyps evi.Evd.evar_hyps then
        Evd.map_evar_info f evi
      else
        let () = subst := Evar.Map.add ev inst !subst in
        let evi' = move_to_goal_hyps vars hyps hyps' !subst evi in
        Evd.map_evar_info f evi'
    in
    let h' = Clenv.{ h with hole_evar = Term.mkEvar (ev, inst) } in
    Evd.add evd ev evi', h' :: holes
  in
  let update f evd = List.fold_left (add_hole f) evd (clenv_holes clenv) in
  ...
  let evd, clenv, c =
    let evd, map =
      if poly then
        ...
        let map c = extend_evars !subst (Vars.subst_univs_level_constr usubst c) in
        let evd = Evd.merge_context_set Evd.univ_flexible evd ctx in
        evd, map
      else
        let evd = Evd.merge_context_set Evd.univ_flexible evd ctx in
        let map c = extend_evars !subst c in
        evd, map
    in
    let evd, holes = update map (evd, []) in
    let clenv = {
        cl_holes = List.rev holes;
        cl_concl = map clenv.cl_concl;
        cl_val = map clenv.cl_val;
        cl_concl_occs = clenv.cl_concl_occs
      } in
    evd, clenv, map c
  in evd, clenv, c

let connect_clenv poly c clenv kont =
  let open Ftactic in
  let tac =
    enter { enter = begin fun gl ->
     let res =
       try Some (connect_hint_clenv poly c clenv gl)
       with Not_found -> None
     in
     match res with
