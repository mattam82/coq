...
module Search = struct
  ...
  let hints_tac_gl hints info kont gl : unit Proofview.tactic =
    ...
    | exception Hints.ModeMatchFailure ->
      Proofview.tclZERO ~info:Exninfo.null StuckClass
    | poss ->
  ...
  let search_tac mst only_classes dep hints depth =
    ...
    in
    let tac_or_stuck sigma gls i ev =
      Proofview.tclOR
       (tac sigma gls i)
     ...
     in
      ...
      let j = List.length gls in
      (tclDISPATCH (List.init j (fun i -> tac_or_stuck sigma gls i (List.nth gls i))))
...
let typeclasses_eauto ?(only_classes=false) ?(st=TransparentState.full)
  ...
  Proofview.tclBIND
    ...
    (* Stuck goals will remain here, we could also shelve them, but this way
       `solve [typeclasses eauto]` can check there are no stuck goals
       remaining. *)
    (fun l -> Proofview.tclUNIT ())

(*       let goals = List.map_filter (fun x -> Option.map Proofview_monad.with_empty_state x) l in
      Proofview.Unsafe.tclNEWGOALS goals. *)
