...
let declare_abstract = ref (fun ~name ~poly_flags ~sign ~secsign ~opaque ~solve_tac env sigma concl ->
...
let cache_term_by_tactic_then ~opaque ~name_op ?(goal_type=None) tac tacK =
  ...
  Proofview.tclProofInfo [@ocaml.warning "-3"] >>= fun (name, poly_flags) ->
  ...
  Proofview.Goal.enter begin fun gl ->
    ...
    let sigma, lem, args, safe =
      !declare_abstract ~name ~poly_flags ~sign ~secsign ~opaque ~solve_tac env sigma concl
    in
