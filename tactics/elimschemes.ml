...
let optimize_non_type_induction_scheme kind dep sort id _ ind =
  let env = Global.env () in
  let sigma = Evd.from_env ~id env in
...
let build_induction_scheme_in_type dep sort ind id =
  let env = Global.env () in
  let sigma = Evd.from_env ~id env in
  let sigma, pind = Evd.fresh_inductive_instance env sigma ind in
  let sigma, c = build_induction_scheme env sigma pind dep sort in
...
let rect_scheme_kind_from_type =
  declare_individual_scheme_object "_rect_nodep"
    (fun id _ x -> build_induction_scheme_in_type false InType x id, Safe_typing.empty_private_constants)
...
let rect_dep_scheme_kind_from_type =
  declare_individual_scheme_object "_rect" ~aux:"_rect_from_type"
    (fun id _ x -> build_induction_scheme_in_type true InType x id, Safe_typing.empty_private_constants)
...
let build_case_analysis_scheme_in_type dep sort ind id =
  let env = Global.env () in
  let sigma = Evd.from_env ~id env in
...
let case_scheme_kind_from_type =
  ...
  (fun id _ x -> build_case_analysis_scheme_in_type false InType x id, Safe_typing.empty_private_constants)
...
let case_dep_scheme_kind_from_type =
  ...
  (fun id _ x -> build_case_analysis_scheme_in_type true InType x id, Safe_typing.empty_private_constants)

let case_dep_scheme_kind_from_type_in_prop =
  declare_individual_scheme_object "_casep_dep"
  (fun id _ x -> build_case_analysis_scheme_in_type true InProp x id, Safe_typing.empty_private_constants)
