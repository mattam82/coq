...
let define ?loc internal role id c ~poly env uctx sch =
  ...
  let variances = if SortPolyFlags.cumulative poly then Some Entries.Infer_variances else None in
...
let rec define_individual_scheme_base ?loc kind suff f ~internal idopt (mind,i as ind) eff =
  ...
  let poly = SortPolyFlags.make ~level_polymorphic:poly ~cumulative ~sort_polymorphic:false in
