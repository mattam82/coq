...
let init_status_ustate ?(position=Position.InType) ?(udecl : UState.sort_poly_decl option) ustate =
  ...
  | None ->
    ...
    | Some udecl ->
      ...
      let us = udecl.UState.sort_poly_decl_instance and variances = udecl.UState.sort_poly_decl_variances in
...
let init_status ?(position=Position.InType) ?(udecl : UState.sort_poly_decl option) sigma =
