...
let minimize
  ...
  let (vars', variances), us' =
    normalize_context_set ~variances ~partial uctx.universes uctx.local uctx.univ_variables
...
let minimize ?(variances = InferCumulativity.empty_level_variances) ~partial uctx =
  if CDebug.get_flag disable_minim then uctx, variances
  else minimize ~variances ~partial uctx
