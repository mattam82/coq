...
let new_pure_evar ?(src=default_source) ?(filter = Filter.identity)
...
let new_evar_instance ?src ?filter ?candidates ?store ?naming
...
let new_evar ?src ?filter ?candidates ?store ?naming ?future_goal ?principal ?hypnaming env evd typ =
