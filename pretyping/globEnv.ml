...
type 'a obj_interp_fun =
  ?loc:Loc.t -> SortPolyFlags.t -> t -> Evd.evar_map -> Evardefine.type_constraint ->
...
let interp_glob_genarg ?loc poly_flags env sigma ty arg =
  let open Genarg in
  let GenArg (Glbwit tag, arg) = arg in
  let interp = ConstrInterp.obj tag in
  interp ?loc poly_flags env sigma ty arg
