...
let magicaly_constant_of_fixbody env bd = function
  ...
  | Name.Name id ->
	...
	in
	let inst = List.map (fun u -> Univ.Universe.make
	  (Univ.LMap.find (Option.get (Univ.universe_level u)) subst)) u in
...
let rec whd_state_gen ?csts refold flags env sigma =
  ...
  let rec whrec cst_l (x, stack as s) =
    ...
    | Rel _ | Var _ | Const _ | Proj _ | LetIn _ -> fold ()
...
let whd_betaiota_preserving_vm_cast env sigma t =
   ...
   and whrec (x, stack as s) =
	 ...
	 whrec (d, Zcase (ci,p,lf,None) :: stack)
	  ...
	  | None -> assert false
	  | Some pb -> whrec (c, 
			      Zproj (pb.Declarations.proj_npars, 
				     pb.Declarations.proj_arg, p) :: stack))
