...
let refresh_universes ?(status=univ_rigid) ?(onlyalg=false) ?(refreshset=false)
  ...
  and refresh_term_evars onevars top t =
    ...
    | App (f, args) when top && isEvar !evdref f -> 
       let f' = refresh_term_evars true false f in
       let args' = Array.map (refresh_term_evars onevars false) args in
    ...
    | Evar (ev, a) when onevars ->
      ...
      if ty == ty' then t
      else (evdref := Evd.add !evdref ev {evi with evar_concl = EConstr.to_constr !evdref ty'}; t)
    | Sort s ->
       (match ESorts.kind !evdref s with
        | Type u when not (Univ.Universe.is_levels u) ->
           refresh_sort Evd.univ_flexible ~direction:false s
        | _ -> t)
    | _ -> EConstr.map !evdref (refresh_term_evars onevars false) t
  ...
  let t' = 
    ...
    else refresh_term_evars false true t
  in !evdref, t'
