...
let mkExistential env ?(src=(Loc.ghost,Evar_kinds.InternalHole)) 
    ?(var=Univ.Levels.Invariant) evdref =
  let e, u = e_new_type_evar env evdref univ_flexible_alg var ~src:src in e
...
let build_tycon loc env tycon_env subst tycon extenv evdref t =
  let t,tt = match t with
	...
	let impossible_case_type, u =
	  e_new_type_evar (reset_context env) evdref univ_flexible_alg 
	    Univ.Levels.CoVariant
	    ~src:(loc,Evar_kinds.ImpossibleCase) in
...
(*FIXME TRY *)
  ...
  let sigma, s = Evd.new_sort_variable univ_flexible_alg Univ.Levels.CoVariant sigma in
...
let prepare_predicate loc typing_fun env sigma tomatchs arsign tycon pred =
  let preds =
	...
	| None -> 
	  let sigma, (t, _) = 
	    new_type_evar env sigma univ_flexible_alg
	      Univ.Levels.Invariant ~src:(loc, Evar_kinds.CasesType false) in
	    sigma, t
	in
    ...
    | Some rtntyp, _ ->
      ...
      let sigma, newt = new_sort_variable univ_flexible_alg Univ.Levels.Invariant sigma in
      let evdref = ref sigma in
