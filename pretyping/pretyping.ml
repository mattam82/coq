...
let pretype_id loc env sigma (lvar,unbndltacvars) id =
  ...
  try
    let (n,b,typ) = lookup_rel_id id (rel_context env) in
      { uj_val  = mkRel n; uj_type = lift n typ }, annot_of_body b
  with Not_found ->
  ...
  try
    let (_,b,typ) = lookup_named id env in
      ({ uj_val  = mkVar id; uj_type = typ }, annot_of_body b)
  with Not_found ->
...
let rec pretype (tycon : type_constraint) env evdref lvar = function
  ...
  | GApp (loc,f,args) ->
	  ...
	  let npars = inductive_nparams ind in
		...
		let ty = evd_comb1 (Coercion.inh_coerce_to_prod loc env) evdref ty in
	  	...
	  	let (ind',pars) = dest_ind_family indf in
	  	  if ind = ind' then Constr.args_of pars
	...
	let resj = evd_comb1 (Coercion.inh_app_fun env) evdref resj in
	  ...
	  | Prod ((na,(rel,_)),c1,c2) ->
	    ...
	    in
	    let value, typ = mkApp (j_val resj, [| relc |], [| ujval |]), subst1 ujval c2 in
	      apply_rec env (n+1)
	      { uj_val = value;
		uj_type = typ }
	      candargs rest
    ...
    let resj, relf =
      ...
      | App (f,an,args) ->
	    ...
	    ->
	    let sigma =  !evdref in
	    let c = mkApp (f,an,Array.map (whd_evar sigma) args) in
  ...
  | GLetIn(loc,name,c1,c2)      ->
    ...
    let j', rel = pretype tycon (push_rel var env) evdref lvar c2 in
      { uj_val = mkLetIn ((name, rel), j.uj_val, t, j'.uj_val) ;
  	...
  	let nar = List.length arsgn in
  	  (match po with
	     ...
	     let fj, _ = pretype (mk_tycon fty) env_f evdref lvar d in
  		 ...
  		 Term.mkCase (ci, p, cj.uj_val,[|f|]) in
  ...
  | GIf (loc,c,(na,po),b1,b2) ->
	...
	let bj, rel = pretype (mk_tycon pi) env_c evdref lvar b in
	  it_mkLambda_or_LetIn bj.uj_val cs.cs_args, rel in
      let b1, rel = f cstrs.(0) b1 in
      let b2, _ = f cstrs.(1) b2 in
      let v =
	let ind,_ = dest_ind_family indf in
	let ci = make_case_info env ind IfStyle in
	let pred = nf_evar !evdref pred in
	  Typing.check_allowed_sort env !evdref ind cj.uj_val pred;
	  Term.mkCase (ci, pred, cj.uj_val, [|b1;b2|])
      in
	({ uj_val = v; uj_type = p }, rel)
  ...
  | GCast (loc,c,k) ->
	...
	let cj, rel' = match k with
	  | VMcast ->
 	    let cj, rel' = pretype empty_tycon env evdref lvar c in
	    let cty = nf_evar !evdref cj.uj_type in
...
let understand_ltac ?(resolve_classes=false) expand_evar sigma env lvar kind c =
  let evd, (c, _) = ise_pretype_gen expand_evar false resolve_classes sigma env lvar kind c in
    evd, c

let understand_tcc ?(resolve_classes=true) sigma env ?expected_type:exptyp c =
  let evd, (c, rel) = 
    ise_pretype_gen true false resolve_classes sigma env ([],[]) (OfType exptyp) c in
    (evd, c), rel
