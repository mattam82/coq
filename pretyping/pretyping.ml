...
let eval_pretyper self ~program_mode ~allowed_evars ~poly resolve_tc tycon env sigma t =
  ...
  | GCast (c, t) ->
    self.pretype_cast self (c, t) ?loc ~program_mode ~allowed_evars ~poly resolve_tc tycon env sigma
  | GInt n ->
...
struct
  ...
  let pretype_rec self (fixkind, names, bl, lar, vdef) =
    ...
    let pretype ~allowed_evars tycon env sigma c = eval_pretyper self ~program_mode ~allowed_evars ~poly resolve_tc tycon env sigma c in
    let pretype_type ~allowed_evars tycon env sigma c = eval_type_pretyper self ~program_mode ~allowed_evars ~poly resolve_tc tycon env sigma c in
                ...
                (* There is a self-reference to the unrefined body
                  ...
                  if Array.exists (fun ev -> occur_evar sigma (fst ev) j.uj_val) bodies then
                    ...
                    user_err ?loc Pp.(str "Cannot refine the fixpoint body evar " ++
                      Termops.pr_existential_key sigma (fst bodies.(i)) ++
