...
Proof.
split.
  revert n v1 v2; refine (@ind2 _ _ _ _ _); simpl; intros.
    reflexivity.
...
Proof.
unfold nth_order; refine (@indS _ _ _ _); now simpl.
Qed.
...
subst k2; induction k1.
  generalize dependent n. apply caseSP ; intros. now simpl.
  generalize dependent n. refine (@caseSP _ _ _) ; intros. now simpl.
Qed.
...
Proof.
refine (@Fin.indS _ _ _); intros.
  revert n v; refine (@caseSP _ _ _); simpl; intros. now destruct t.
  revert p H.
...
subst p2; induction p1.
  revert n v; refine (@caseSP _ _ _); now simpl.
  revert n v p1 IHp1; refine (@caseSP _  _ _); now simpl.
Qed.
...
refine (@ind2 _ _ _ _ _); simpl.
  exact (Fin.case0P _).
  intros n v1 v2 H a b p; revert n p v1 v2 H; refine (@Fin.caseSP _ _ _);
    now simpl.
Qed.
