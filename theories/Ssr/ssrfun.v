...
Definition apply@{s s'|u v|} (aT : Type@{s|u}) (rT : Type@{s'|v}) (f : aT -> rT) x (u : option@{s s'|u} aT) := if u is Some y then f y else x.
...
Definition nesym := fun A (x y : A) (neq : x <> y) eq => neq (eq_sym eq).
Definition etrans := @eq_trans.
Definition congr1 {A B} f {x y} e := @f_equal A B f x y e.
...
Definition tag := @proj1.
Definition tagged : forall w, T_(tag w) := @proj2 I [eta T_].
...
Proof.
  by move=> fK x y=> /(f_equal g); rewrite !fK=> /(f_equal (Option.default x)).
...
Proof. by move=> x y /(f_equal (Option.default x)). Qed.
...
Proof. by move=> injfh x y /(f_equal f) /injfh. Qed.
