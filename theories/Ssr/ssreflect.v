...
Require Import Types.Bool. (* For bool_scope delimiter 'bool'. *)
...
(* #[universes(template)] *)
Structure unlockable@{s|u|} (T : Type@{s|u}) v : Type@{s|u} := Unlockable {unlocked : T; unlocked_eq : unlocked ≡ v}.
...
Definition locked_with (k : unit@{Type|}) := let: tt := k in fun T x => x : T.
...
Fixpoint nary_congruence_statement (n : nat@{Type|})
...
Set Debug "backtrace".
Lemma nary_congruence (n : nat@{Type|}) (k := fun B e => forall y : B, (e y y : Prop)) :
