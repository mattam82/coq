...
Inductive sigma@{s s' s''|u v| } {A:Type@{s|u}} (P:A -> Type@{s'|v}) :
  Type@{s''|max(u,v)} :=
    existP : forall x:A, P x -> sigma P.
(** listings: end **)

Definition ex@{s|u|} {A:Type@{s|u}} (P:A -> Prop) : Prop := sigma@{_ _ _|_ Set} P.
...
Notation "'Σ' x .. y , B" := (sigma (fun x => .. (sigma (fun y => B)) ..))
  (at level 200, x binder, right associativity,
   format "'[' 'Σ'  '/  ' x  ..  y ,  '/  ' B ']'")
  : type_scope.

Notation "( x , .. , y , z )" := (existP _ x .. (existP _ y z) ..).

Definition sigma2@{s s' s''|u v v'| } (A:Type@{s|u})
  (P :A -> Type@{s'|v}) (Q :A -> Type@{s'|v'}) : Type@{s''|max(u,v,v')} :=
  Σ (x : A) (p : P x), Q x.

Definition exist2P@{s s' s''|u v v'| } (A:Type@{s|u})
  (P :A -> Type@{s'|v}) (Q :A -> Type@{s'|v'}) (x : A) (p : P x) (q : Q x) : sigma2 A P Q := (x, p, q).


Definition ex2@{s|u| } {A : Type@{s|u}} (P Q : A -> Prop) := sigma2@{s Prop Prop|u Set Set} A P Q.
Notation ex_intro2 := exist2P@{_ Prop Prop|_ Set Set}.
...
Definition ex_of_ex2@{s|u|} {A : Type@{s|u}} {P Q : A -> Prop} (X : ex2 P Q) : ex P
  := let '(x, p, _) := X in ex_intro P x p.
...
Definition sigma_elim@{s s' s''| u u' u''|} :
  forall [A : Type@{s|u}] [P : A -> Type@{s'|u'}]
    (P0 : sigma@{s s' s''| u u'} P -> Type@{s''|u''}),
    (forall (x : A) (p : P x), P0 (existP P x p)) -> forall s : sigma P, P0 s :=
  fun A P P0 f s =>
    match s as s0 return (P0 s0) with
          | existP _ x p => f x p
    end.

Definition sig@{s|u|} {A:Type@{s|u}} (P:A -> Prop) : Type@{s|u} := sigma@{s Prop s| u Set} P.
...
Definition sig2 {A:Type} (P Q:A -> Prop) : Type := sigma2 A P Q.
...
Record sigT@{s|u v|} {A:Type@{s|u}} (B:A -> Type@{s|v}) : Type@{s|max(u,v)} := existT
  { projT1 : A; 
    projT2 : B projT1 }.
...
Definition sigT_rect@{s | u u' u''|} :
  forall [A : Type@{s|u}] [P : A -> Type@{s|u'}]
    (P0 : sigT P -> Type@{s|u''}),
    (forall (x : A) (p : P x), P0 (existT A P x p)) -> forall s : sigT P, P0 s :=
  fun A P P0 f s =>
    match s as s0 return (P0 s0) with
          | existT _ _ x p => f x p
    end.
...
Arguments sigT (A B)%_type.
Arguments sigT2 (A B C)%_type.
...
Section Subset_projections.
  ...
  Definition proj1_sig@{s s' | u v |} {A:Type@{s|u}} {P : A -> Type@{s'|v}}
    (e:sigma@{s s' s | u v} P) := match e with | existP _ a b => a end.

  Definition proj2_sig@{u|} (A:Type@{u}) (P : A -> Prop)
    (e:sigma@{Type Prop Type | u Set} P) :=
    match e return P (proj1_sig@{_ _ | _ Set} e) with
    | exist _ a b => b
    end.
...
Definition sig_of_sig2 {A : Type} {P Q : A -> Prop} (X : sig2 P Q) : sig P
  := exist P
           (let '(a, _, _) := X in a)
           (match X as s return (P (let '(a, _, _) := s in a)) with | (x, p, _) => p end).
...
Section Subset_projections2.
  ...
  Definition proj3_sig (e : sig2 P Q) :=
    match e as e return Q (proj1_sig (sig_of_sig2 e)) with | (a, b, c) => c end.
...
Definition sig_eta@{u|} {A:Type@{u}} {P:A->Prop} (p : { a : A | P a })
  : p = exist P (proj1_sig p) (@proj2_sig@{u} A P p).
 ...
 *              (pq : { p : u1 = v1 & rew p in u2 = v2 } : Prop)
 *     : (u1; u2) = (v1; v2).
 *   Proof.
 *     destruct pq as [p q].
 *     destruct q; simpl in *.
 ...
 *              (pq : @sigT@{Prop| _ _} (u.1 = v.1) (fun p => rew p in u.2 = v.2) : Prop)
 *     : u = v.
 *   Proof.
 ...
 *   Definition eq_sigT_uncurried_iff {A :Type} {P:A -> Type}
 *              (u v : { a : A & P a })
 *     : u = v <-> (@sigT@{Prop|_ _} (u.1 = v.1) (fun p => rew p in u.2 = v.2)).
 *   Proof.
 *     split; [ intros H; destruct H; exists eq_refl; reflexivity | apply eq_sigT_uncurried ].
 *   Defined.
 * 
 ...
 *              (pq : { p : proj1_sig u = proj1_sig v | rew p in proj2_sig u = proj2_sig v } : Prop)
 *     : u = v.
 *   Proof.
 *     destruct u as [u1 u2], v as [v1 v2]; simpl in *.
