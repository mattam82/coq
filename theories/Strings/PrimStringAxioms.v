From Corelib Require Import BinNums PosDef IntDef NumberNotations.
From Corelib Require Export PrimInt63 Uint63Axioms.
From Corelib Require Export PrimString Peano.
From Corelib.Properties Require Export List.
...
Definition to_list (s : string) : list char63 :=
  map (fun i => get s (of_nat i)) (seq 0 (to_nat (length s))).
...
Axiom make_spec :
  forall (i : int) (c : char63),
    to_list (make i c) =
      List.repeat (PrimInt63.land c 255%uint63)
...
Axiom compare_spec :
  forall (s1 s2 : string),
    compare s1 s2 =
      list_compare char63_compare (to_list s1) (to_list s2).
