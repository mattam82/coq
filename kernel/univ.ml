...
let pr_universe_context (ctx, cst) =
  if ctx = [] && Constraint.is_empty cst then mt() else
    pr_universe_level_list ctx ++ str " |= " ++ v 1 (pr_constraints cst)
...
let universe_context_set_of_set l =
  (l, empty_constraint)

let universe_context_set_of_ulist l =
  (UList.levels l, empty_constraint)
...
let hcons_universe_context (v, c) = (LList.hcons v, hcons_constraints c)
