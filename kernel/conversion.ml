...
let checked_sort_cmp_universes _env pb s0 s1 state =
  match pb with
...
let check_cumul_instances_univs ~flex:_ cv_pb ~nargs variance u1 u2 (elims, univs) =
  let qcsts, ucsts = get_cumulativity_constraints cv_pb ~nargs variance u1 u2 in
  QGraph.check_constraints (Sorts.QCumulConstraints.to_elims qcsts) elims && (UGraph.check_constraints ucsts univs)

(* general conversion and inference functions *)
let check_cumul_instances ~flex:_ ~nargs cv_pb variance u1 u2 (elims, univs as state) =
  let qcsts, ucsts = get_cumulativity_constraints cv_pb ~nargs variance u1 u2 in
  if QGraph.check_constraints (Sorts.QCumulConstraints.to_elims qcsts) elims && (UGraph.check_constraints ucsts univs) then Result.Ok state
  else Result.Error None
...
let conv_inst u1 u2 ((elims, univs : QGraph.t * UGraph.t) as gs) = if UGraph.check_eq_instances elims univs u1 u2 then Result.Ok gs else Result.Error ()
...
let cumul_head_instances env univs cv_pb head u1 u2 =
  ...
  | Some (gr, nargs) ->
    ...
    | GlobRef.VarRef _ -> UGraph.check_eq_instances (fst univs) (snd univs) u1 u2
    with UCompare.MustExpand -> false)
  | None -> UGraph.check_eq_instances (fst univs) (snd univs) u1 u2
...
let leq_constr_univs env m n =
  if m == n then true
    ...
    let quals = Environ.qualities env in
    let eq_instances = cumul_head_instances env (quals, univs) CONV in
    let leq_instances = cumul_head_instances env (quals, univs) CUMUL in
