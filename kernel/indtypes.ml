  ...
  and check_positive_nested (env,n,ntypes,_ncstrs,_ra_env as ienv) nmr ((mi,u), largs) =
    ...
    let (auxrecparams,auxnonrecargs) =
      ...
      let lc = Array.mapi (fun k cty ->
        let csubst = constructor_subst (fst mi,0) mib u (k+1) in
      ...
      in (nmr',(Rtree.mk_rec [|mk_paths (Nested (NestedInd mi)) irecargs|]).(0))

  and check_positive_nested_primitive (env,n,ntypes,ncstrs,ra_env) nmr (c, largs) =
...
let compute_projections (kn, i as ind) mib =
  ...
  let substc = Inductive.constructor_subst ind mib u 1 in
